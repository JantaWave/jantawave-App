...
export default function LeaderActivityScreen() {
  ...
  const fetchActivityData = async (isRefresh = false) => {
    try {
      ...
      if (activeTab === 'streams') {
        ...
        const data = await getUserStreams(user.id, 5, currentCursor);
  ...
  const renderPostItem = ({ item, index }: { item: any; index: number }) => (
    <PostCard
      ...
      author={{
        ...
        first_name: item.user?.first_name || user?.first_name,
        last_name: item.user?.last_name || user?.last_name,
        avatar_url: item.user?.avatar_url || user?.avatar_url,
      }}
      isDark={isDark}
      isActiveVideo={item.media_type === 'video' && index === activeVideoIndex}
      ...
      {activeTab === 'posts' ? (
        <FlatList
          ...
          onViewableItemsChanged={({ viewableItems }) => {
            const firstVisibleVideo = viewableItems.find((v) => v.item?.media_type === 'video');
            setActiveVideoIndex(firstVisibleVideo ? (firstVisibleVideo.index ?? null) : null);
          }}
          viewabilityConfig={{
        ...
        <FlatList
          ...
          onViewableItemsChanged={() => {
            // No-op for streams tab - keeps the prop consistent with posts tab
          }}
          viewabilityConfig={{
            itemVisiblePercentThreshold: 70,
          }}
